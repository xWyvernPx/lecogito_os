...
import { useBlogSearch, useSeries, useCategories, useUsers } from "@/services";
import {
  blogDtoToPost,
  serieDtoToLocal,
  DEFAULT_SEARCH_REQUEST,
  DEFAULT_BLOG_CRITERIA,
  type BlogPost,
} from "./utils";
...
export const BlogArchive: React.FC<BlogArchiveProps> = ({
  ...
  // Fetch data from API
  const { data: blogsData } = useBlogSearch(
    {authorIds: [], categoryIds: [], serieIds: []},
    DEFAULT_SEARCH_REQUEST
  );
  const { data: seriesData } = useSeries(DEFAULT_SEARCH_REQUEST);
  const { data: categoriesData } = useCategories(DEFAULT_SEARCH_REQUEST);
  const { data: usersData } = useUsers(DEFAULT_SEARCH_REQUEST);
  ...
  const categories = useMemo(() => {
    return categoriesData?.rows?.map((cat) => cat.name) || [];
  }, [categoriesData]);

  const authors = useMemo(() => {
    return usersData?.rows?.map((user) => user.fullName) || [];
  }, [usersData]);
  ...
  const filteredPosts = posts.filter((post) => {
    ...
    if (selectedAuthor !== "All" && post.author !== selectedAuthor)
      return false;
    if (selectedSeries !== "All" && post.serieId !== selectedSeries)
      return false;
    return true;
  });
    ...
    <div className="flex flex-col h-full bg-white text-stone-800">
      <ShareDialog
        ...
        meta={{
          ...
          description:
            "Explore the latest scrolls and wisdom from the Cogito OS community.",
      ...
      <div className="h-12 bg-[#f4f1ea] border-b border-stone-300 flex items-center justify-between px-4 select-none shrink-0">
        <div className="flex items-center gap-1">
          <button
            onClick={onBack}
            className="p-1.5 hover:bg-stone-200 rounded mr-2 text-stone-600"
          >
            <ChevronLeft size={18} />
          </button>
          ...
          <div className="h-6 w-px bg-stone-300 mx-2" />
          <button
            className="p-1.5 hover:bg-stone-200 rounded font-bold"
            aria-label="Bold"
          >
            <Bold size={14} />
          </button>
          <button
            className="p-1.5 hover:bg-stone-200 rounded italic"
            aria-label="Italic"
          >
            <Italic size={14} />
          </button>
          <button
            className="p-1.5 hover:bg-stone-200 rounded underline"
            ...
            <AnimatePresence>
              {isCatOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute top-full left-0 mt-1 w-48 bg-white border border-stone-200 shadow-xl rounded-md z-20 py-1"
                >
                  {categoryOptions.map((cat) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 hover:bg-stone-100 text-xs flex justify-between items-center"
                    >
                      <span>{cat}</span>
            ...
            <AnimatePresence>
              {isAuthOpen && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute top-full left-0 mt-1 w-48 bg-white border border-stone-200 shadow-xl rounded-md z-20 py-1 max-h-60 overflow-y-auto"
                >
            ...
            <AnimatePresence>
              {isSeriesOpen && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 5 }}
                  className="absolute top-full left-0 mt-1 w-64 bg-white border border-stone-200 shadow-xl rounded-md z-20 py-1 max-h-60 overflow-y-auto"
                >
                  {seriesOptions.map((sId) => (
                    <button
                      key={sId}
                      onClick={() => {
                        setSelectedSeries(sId);
                        setIsSeriesOpen(false);
                      }}
                      className="w-full text-left px-3 py-2 hover:bg-stone-100 text-xs flex justify-between items-center"
                    >
              ...
              >
                <div className="w-full md:w-48 aspect-video md:aspect-[4/3] bg-stone-200 rounded overflow-hidden shrink-0 border border-stone-200 shadow-sm group-hover:shadow-md transition-all">
                  {post.thumbnail ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-[#e8e4d9] text-stone-400 font-mono text-xs p-4 text-center">
                      NO IMAGE
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 flex items-center gap-2 text-xs font-bold text-stone-500">
                    <div className="w-5 h-5 rounded-full bg-stone-200 flex items-center justify-center border border-stone-300">
                      <User size={10} />
                    </div>
                    {post.author}
                  </div>
